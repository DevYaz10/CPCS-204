...
public class LinkedList {
    // head is a reference variable (i.e. class variable) to the first node of the
    // list
    ...
    private LLnode insert(LLnode head, int data) { // Receives reference variable and data
        ...
        if (head == null || head.getData() > data) {// If head is null i.e. there is no node or if there
            ...
            // only node and then assigns its address to the reference variable ‘head’ so
            // that
            // head will be pointing to this newly created node which has now become the
            // first node in the list i.e. it will always add new node to the front of link
            // list
            return head;
            ...
            LLnode newNode = new LLnode(data, helpPtr.getNext()); // Creates a newNode and
                                                                  // sets its ‘next’ field to point to the successor
                                                                  // node
    ...
    private LLnode delete(LLnode head, int data) {
        ...
        if (!isEmpty()) {
            ...
            // we found it. Delete by skipping the node and making head point to the next
            // node.
            if (head.getData() == data) {
                head = head.getNext(); // Head will now point to the next node in the list
            } // ELSE, the data is perhaps somewhere else in the list...so we must traverse
              // and
              // look for it
            else {
                // We need to traverse to find the data we want to delete...so we need a help
                // ptr
        ...
        return head; // If list becomes empty, it returns null as the value of reference variable
                     // ‘head’
    }
    ...
    // Write code to
    // -find, if list contains only one node, then display the message "Cannot find
    // data of second node; list has only one node
    // -otherwise return data of second node in the list
    private int Data2ndNode(LLnode head) {
        if (head.getNext() == null) {
            System.out.println(">    Cannot find data of second node; list has only one node");
            return head.getData(); // or some sentinel value, see note below
        }
    ...
    // Write code to
    // -find, if list contains only one node, then display the message "Cannot find
    // data of second last node; list has only one node
    // -otherwise traverse every node of the list and return the data of second last
    // node in the list
    private int Data2ndLastNode(LLnode head) {
        // Handle the single-node case
        if (head.getNext() == null) {
            System.out.println(">    Cannot find data of second last node; list has only one node");
            return head.getData(); // or some sentinel value, see note below
        }

        // Traverse until helpPtr is the second-to-last node
        LLnode helpPtr = head;
    ...
    // Method to convert all odd value nodes into even value nodes in the list and
    // vice versa
